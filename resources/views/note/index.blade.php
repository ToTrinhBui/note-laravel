<x-app-layout>
    <div class="note-container p-8">
        <a href="{{ route('note.create') }}" class="new-note-btn inline-block">
            <div
                class=" bg-yellow-400 h-9 w-9 flex items-center justify-center border-black border">
        ...
        <div class="notes py-6 grid grid-cols-[repeat(auto-fit,minmax(250px,1fr))] gap-12">
            @foreach ($notes as $note)
                <div class="note p-4 my-1 border border-black shadow-[5px_5px_0px_#000000]">
                    <div class="note-buttons flex flex-row-reverse items-center gap-1">
                        <a href="{{ route('note.show', $note) }}" class="note-edit-button w-8 h-8 flex items-center justify-center rounded hover:bg-[#C8A0FB]">
                        ...
                        <a href="{{ route('note.edit', $note) }}" class="note-edit-button w-8 h-8 flex items-center justify-center rounded hover:bg-[#C8A0FB]">
                            <svg width="20px" height="20px" viewBox="0 0 24 24" fill="none"
                                ...
                                <g id="SVGRepo_iconCarrier">
                                    ...
                                    <path
                                        ...
                                        stroke-linejoin="round"></path>
                                    <path d="M15.3097 5.30981L18.7274 8.72755" stroke="#323232" stroke-width="2"
                                        stroke-linecap="round" stroke-linejoin="round"></path>
                                </g>
                            </svg>
                        </a>
                        <form action="{{ route('note.destroy', $note) }}" method="POST" class="w-8 h-8 flex items-center justify-center rounded hover:bg-[#C8A0FB]">
                            @csrf
                            @method('DELETE')
